...
from Crypto.Hash import SHA256, SHA1, RIPEMD160
from AddressCoder import hash256, hash160
...
def op_ripemd160(stack):
    if len(stack) < 1:
        return False
    element = stack.pop()
    stack.append(RIPEMD160.new(element).digest())
    return True
...
def op_sha1(stack):
    if len(stack) < 1:
        return False
    element = stack.pop()
    stack.append(SHA1.new(element).digest())
    return True
...
def op_sha256(stack):
    if len(stack) < 1:
        return False
    element = stack.pop()
    stack.append(SHA256.new(element).digest())
    return True
...
def op_checksig(stack, z):
    ...
    try:
        point = S256Point.parse(pubkey_sec)
        sig = Signature.parse(signature_der)
